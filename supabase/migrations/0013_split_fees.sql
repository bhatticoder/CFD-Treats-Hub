-- ==========================================================================
-- Migration 0013: Split Fees (Regular vs Preorder) & Flat Delivery
-- ==========================================================================
...
alter table public.campuses
  add column if not exists regular_delivery_fee numeric not null default 0 check (regular_delivery_fee >= 0),
  add column if not exists regular_platform_fee numeric not null default 5 check (regular_platform_fee >= 0),
  add column if not exists regular_gst numeric not null default 5 check (regular_gst >= 0),
  add column if not exists regular_cod_charge numeric not null default 30 check (regular_cod_charge >= 0),
  
  add column if not exists preorder_delivery_fee numeric not null default 0 check (preorder_delivery_fee >= 0),
  add column if not exists preorder_platform_fee numeric not null default 5 check (preorder_platform_fee >= 0),
  add column if not exists preorder_gst numeric not null default 5 check (preorder_gst >= 0),
  add column if not exists preorder_cod_charge numeric not null default 30 check (preorder_cod_charge >= 0);

-- Update place_order (Regular items)
...
begin
  ...
  -- Load Regular Fees (Flat per order)
  v_del_fee := coalesce(v_campus.regular_delivery_fee, 0);
...
begin
  ...
  -- Load Preorder Fees (Flat per order)
  v_del_fee := coalesce(v_campus.preorder_delivery_fee, 0);
  ...
  insert into public.audit_log (actor_id, actor_role, action, entity, entity_id, campus_id, detail) values (v_user_id, 'customer', 'place_preorder', 'order', v_order_id::text, v_campus_id, jsonb_build_object('total', v_grand, 'items', p_items));
  return jsonb_build_object('id', v_order_id, 'order_number', v_order_num);
end;
$$;
