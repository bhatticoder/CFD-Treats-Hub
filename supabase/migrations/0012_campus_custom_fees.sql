-- ==========================================================================
-- Migration 0012: Campus Custom Fees
-- ==========================================================================

-- 1. Add fee columns to campuses
alter table public.campuses 
  add column if not exists platform_fee numeric not null default 5 check (platform_fee >= 0),
  add column if not exists cod_charge numeric not null default 30 check (cod_charge >= 0),
  add column if not exists gst_percentage numeric not null default 5 check (gst_percentage >= 0);

-- 2. Update place_order to use dynamic campus fees
...
begin
  ...
  if not v_campus.shift_active then
    raise exception 'Shift is not active — ordering is closed for today';
  end if;

  -- Set dynamic fees
  v_platform := coalesce(v_campus.platform_fee, 5);
  v_gst_pct := coalesce(v_campus.gst_percentage, 5);

  -- COD charge
  if p_payment_method = 'cod' then
    v_cod_charge := coalesce(v_campus.cod_charge, 30);
  end if;
...
-- 3. Update place_preorder to use dynamic campus fees
